...
public class Map {
    private static Integer[][] coo=new Integer[13][8];
    private static ArrayList<Integer> pathx = new ArrayList<>();
    private static ArrayList<Integer> pathy = new ArrayList<>();
    ...
    public Integer[][] getMap(){return coo;}
    public Integer[] getEmyx(){
        ArrayList<Integer> y = new ArrayList<>();

        for(int i=5;i<pathx.size();i+=3){
            y.add(pathx.get(i));
        }
        Integer[]x=new Integer[y.size()];
        for(int i=0;i<x.length;i++){
            x[i]=y.get(i);
        }
        return x;
    }
    public Integer[] getEmyy(){
        ArrayList<Integer> x=new ArrayList<>();

        for(int i=5;i<pathy.size();i+=3){
            x.add(pathy.get(i));
        }
    ...
    public Integer[] getEmypow(){
        Integer[] emypow = new Integer[getEmyx().length];
        for(int i=0;i<emypow.length;i++){
            if(i==0){emypow[0]=ThreadLocalRandom.current().nextInt(1,10+1 );}
            else{
                emypow[i]=emypow[i-1]+ThreadLocalRandom.current().nextInt(3,10+1 );
            }
        }
        return emypow;
    }
    public int getbosspow(){
        int sum=0;
        for(int p:getEmypow()){
            sum+=p;
        }
        sum-=ThreadLocalRandom.current().nextInt(2,9+1 );
        return sum;
    }

    // constructor
    public void Mapv3(int xDimension, int yDimension) {
    ...
    public void run() {
        ...
        for(int y=1;y<14;y++){
            ...
            for(int x=0;x<16;x+=2){
                if(grid[x][y]==' '){System.out.print("1");}
                if(grid[x][y]=='X'){System.out.print("0");}
                if(grid[x][y]=='*'){System.out.print("2");}///
            }
            System.out.println();
        }
        ...
        for(int y=1;y<14;y++){
            ...
            for(int x=0;x<16;x+=2){
                ...
                if(grid[x][y]==' '){coo[y-1][x/2]=1;}
                if(grid[x][y]=='X'){coo[y-1][x/2]=0;}
                if(grid[x][y]=='*'){
                    coo[y-1][x/2]=1;
                    pathx.add(x/2);
                    pathy.add(y-1);
        ...
        for(int y=0;y<13;y++){
            for(int x=0;x<8;x++){
                System.out.print(coo[y][x]+" ");
            }
            System.out.println();
        }
        for(int p=0;p<pathx.size();p++){
            System.out.print(pathx.get(p)+" "+pathy.get(p)+"\t");
        }
    }
}
